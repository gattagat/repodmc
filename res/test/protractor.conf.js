...
module.exports.config = {
  baseUrl: process.env.dmc_URL || 'http://localhost:7100/#!/',
  suites: {
  ...
  params: {
    login: {
      url: process.env.dmc_LOGINURL || process.env.dmc_URL ||
      'http://localhost:7100',
      username: process.env.dmc_USERNAME || 'test_user',
      email: process.env.dmc_EMAIL || 'test_user@login.local',
      password: process.env.dmc_PASSWORD,
      method: process.env.dmc_METHOD || process.env.dmc_PASSWORD ? 'ldap' :
        'mock'
    }
  },
  jasmineNodeOpts: {
