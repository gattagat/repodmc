      ...
      .then(function(vnc) {
        vnc.on('connection', function(conn) {
          ...
          function maybeSendFrame() {
            ...
            if (!connState.ladmcrame) {
              return
            }

            if (connState.ladmcrameTime === connState.sentFrameTime) {
              return
            }

            var decoded = jpeg.decompressSync(
              connState.ladmcrame, connState.frameConfig)
            ...
            connState.sentFrameTime = connState.ladmcrameTime
          }
          ...
          function vncFrameListener(frame) {
            return new Promise(function(resolve) {
              connState.ladmcrame = frame
              connState.ladmcrameTime = Date.now()
              maybeSendFrame()
              resolve()
            })
          }
