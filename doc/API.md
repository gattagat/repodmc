...
dmc API is a REdmcul API which allows you to reserve and release any dmc device. Internally dmc uses [Swagger](http://swagger.io/) interface for its API implementation. For those who don't know about Swagger, Swagger provides specifications for REdmcul apis. By using it you can generate documentation and client SDKs in various language automatically for Swagger-enabled apps. This gives you power to use dmc APIs in any language of your favorite. You can read more about Swagger [here](http://swagger.io/getting-started/).
...
A few [examples](#examples) are also provided. We also have a more advanced example showing [how to use the API with Appium](https://github.com/opendmc/dmc-appium-example).

### Authentication

dmc uses OAuth 2.0 for authentication. In order to use the API, you will first need to generate an access token. Access tokens can be easily generated from the dmc UI. Just go to the **Settings** tab and generate a new access token in **Keys** section. Don't forget to save this token somewhere, you will not be able to see it again.
...
```bash
curl -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/user
```
...
var SWAGGER_URL = 'https://dmc.example.org/api/v1/swagger.json';
...
```sh
curl -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/devices | jq .
```
...
List **all** dmc devices (including disconnected or otherwise inaccessible devices).
...
```bash
curl -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/devices
```
...
```bash
curl -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/devices/xxxxxxxxx
```
...
```bash
curl -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/user/devices
```
...
```bash
curl -X POST --header "Content-Type: application/json" --data '{"serial":"EP7351U3WQ"}' -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/user/devices
```
...
```bash
curl -X DELETE -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/user/devices/{serial}
```
...
_Note that if you haven't added your ADB key to dmc yet, the device may be in unauthorized state after connecting to it for the first time. We recommend you make sure your ADB key has already been set up properly before you start using this API. You can add your ADB key from the settings page, or by connecting to a device you're actively using in the UI and responding to the dialog that appears._
...
```bash
curl -X POST -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/user/devices/{serial}/remoteConnect
```
...
```bash
curl -X DELETE -H "Authorization: Bearer YOUR-TOKEN-HERE" https://dmc.example.org/api/v1/user/devices/{serial}/remoteConnect
```
...
if [ "$dmc_URL" == "" ]; then
    echo "Please specify dmc url using ENV['dmc_URL']"
    exit 1
fi

if [ "$dmc_TOKEN" == "" ]; then
    echo "Please specify dmc token using using ENV['dmc_TOKEN']"
    exit 1
fi
...
function add_device
{
    response=$(curl -X POST -H "Content-Type: application/json" \
                 -H "Authorization: Bearer $dmc_TOKEN" \
                 --data "{\"serial\": \"$DEVICE_SERIAL\"}" $dmc_URL/api/v1/user/devices)
...
function remote_connect
{
    response=$(curl -X POST \
                 -H "Authorization: Bearer $dmc_TOKEN" \
                $dmc_URL/api/v1/user/devices/$DEVICE_SERIAL/remoteConnect)
...
function remove_device
{
    response=$(curl -X DELETE \
                 -H "Authorization: Bearer $dmc_TOKEN" \
                $dmc_URL/api/v1/user/devices/$DEVICE_SERIAL)
