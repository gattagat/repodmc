...
So you've got dmc running via `dmc local` and now you'd like to deploy it to real servers. While there are of course various ways to set everything up, this document will focus on a [systemd](http://www.freedesktop.org/wiki/Software/systemd/) + [Docker](https://www.docker.com/) deployment. Even if you've got a different setup, you should be able to use the configuration files as a rough guide. You can also check some [Setup Examples](https://github.com/opendmc/setup-examples) which uses [Vagrant](https://www.vagrantup.com/) and [Virtual Box](https://www.virtualbox.org/) to create a virtual setup. But before going there, it is highly recommended that you read this document thoroughly.

dmc consists of multiple independent processes communicating via [ZeroMQ](http://zeromq.org/) and [Protocol Buffers](https://github.com/google/protobuf). We call each process a "unit" to match systemd terminology.
...
* You're deploying [opendmc/dmc:latest](https://registry.hub.docker.com/u/opendmc/dmc/). There's also a fixed tag for each release if you're feeling less adventurous.
* You want to access the app at https://dmc.example.org/. Change to the actual URL you want to use.
* You have RethinkDB running on `rethinkdb.dmc.example.org`. Change to the actual address/IP where required.
  - You may also use SRV records by giving the url in `srv+tcp://rethinkdb-28015.skydns.dmc.example.org` format.
* You have two static IPs available for the main communication bridges (or "triproxies"), or are able to figure out an alternate method. In this example we'll use `devside.dmc.example.org` and `appside.dmc.example.org` as easy to remember addresses.
...
* [dmc-provider@.service](#dmc-providerservice)
...
* [dmc-app@.service](#dmc-appservice)
* [dmc-auth@.service](#dmc-authservice)
* [dmc-log-rethinkdb.service](#dmc-log-rethinkdbservice)
* [dmc-migrate.service](#dmc-migrateservice)
* [dmc-notify-hipchat.service](#dmc-notify-hipchatservice)
* [dmc-processor@.service](#dmc-processorservice)
* [dmc-provider@.service](#dmc-providerservice)
* [dmc-reaper.service](#dmc-reaperservice)
* [dmc-storage-plugin-apk@.service](#dmc-storage-plugin-apkservice)
* [dmc-storage-plugin-image@.service](#dmc-storage-plugin-imageservice)
* [dmc-storage-temp@.service](#dmc-storage-tempservice)
* [dmc-triproxy-app.service](#dmc-triproxy-appservice)
* [dmc-triproxy-dev.service](#dmc-triproxy-devservice)
* [dmc-websocket@.service](#dmc-websocketservice)
* [dmc-api@.service](#dmc-apiservice)
* [dmc-groups-engine.service](#dmc-groups-engineservice)
...
These external units are required for the actual dmc units to work.
...
TimeoutStartSec=0
Restart=always
ExecStartPre=/usr/bin/docker pull opendmc/ambassador:latest
ExecStartPre=-/usr/bin/docker kill %p
ExecStartPre=-/usr/bin/docker rm %p
ExecStart=/usr/bin/docker run --rm \
  --name %p \
  -e "AUTHKEY=YOUR_RETHINKDB_AUTH_KEY_HERE_IF_ANY" \
  -p 28015 \
  -e RETHINKDB_PORT_28015_TCP=tcp://rethinkdb.dmc.example.org:28015 \
  opendmc/ambassador:latest
ExecStop=-/usr/bin/docker stop -t 10 %p
```

## Main units

These units are required for proper operation of dmc. Unless mentioned otherwise, each unit can have multiple running instances (possibly on separate hosts) if desired.
...
TimeoutStartSec=0
Restart=always
ExecStartPre=/usr/bin/docker pull opendmc/dmc:latest
...
ExecStart=/usr/bin/docker run --rm \
  ...
  opendmc/dmc:latest \
  dmc app --port 3000 \
    --auth-url https://dmc.example.org/auth/mock/ \
    --websocket-url wss://dmc.example.org/
...
You have multiple options here. dmc currently provides authentication units for [OAuth 2.0](http://oauth.net/2/) and [LDAP](https://en.wikipedia.org/wiki/Lightweight_Directory_Access_Protocol), plus a mock implementation that simply asks for a name and an email address.
...
This is a template unit, meaning that you'll need to start it with an instance identifier. In this example configuration the identifier is used to specify the exposed port number (i.e. `dmc-auth@3200.service` runs on port 3200). You can have multiple instances running on the same host by using different ports.

**NOTE:** Don't forget to change the `--auth-url` option in the `dmc-app` unit. For mock auth, the value should be `https://dmc.example.org/auth/mock/`.

```ini
[Unit]
Description=dmc mock auth
After=docker.service
Requires=docker.service
...
ExecStart=/usr/bin/docker run --rm \
  ...
  -p %i:3000 \
  opendmc/dmc:latest \
  dmc auth-mock --port 3000 \
    --app-url https://dmc.example.org/
ExecStop=-/usr/bin/docker stop -t 10 %p-%i
```
...
**NOTE:** Don't forget to change the `--auth-url` option in the `dmc-app` unit. For OAuth 2.0, the value should be `https://dmc.example.org/auth/oauth/`.

```ini
[Unit]
Description=dmc OAuth 2.0 auth
After=docker.service
Requires=docker.service
...
ExecStart=/usr/bin/docker run --rm \
  ...
  -e "OAUTH_CALLBACK_URL=https://dmc.example.org/auth/oauth/callback" \
  -e "OAUTH_SCOPE=openid email" \
  -p %i:3000 \
  opendmc/dmc:latest \
  dmc auth-oauth2 --port 3000 \
    --app-url https://dmc.example.org/
ExecStop=-/usr/bin/docker stop -t 10 %p-%i
```
...
See `dmc auth-ldap --help` and change one of the unit files above as required.

**NOTE:** Don't forget to change the `--auth-url` option in the `dmc-app` unit. For LDAP, the value should be `https://dmc.example.org/auth/ldap/`.

#### Option D: SAML 2.0

This is one of the multiple options for authentication provided by dmc. It uses [SAML 2.0](http://saml.xml.org/saml-specifications) protocol. If your company uses [Okta](https://www.okta.com/) or some other SAML 2.0 supported id provider, you can use it.

This is a template unit, meaning that you'll need to start it with an instance identifier. In this example configuration the identifier is used to specify the exposed port number (i.e. `dmc-auth@3200.service` runs on port 3200). You can have multiple instances running on the same host by using different ports.

**NOTE:** Don't forget to change the `--auth-url` option in the `dmc-app` unit. For SAML 2.0, the value should be `https://dmc.example.org/auth/saml/`.

```ini
[Unit]
Description=dmc SAML 2.0 auth
After=docker.service
Requires=docker.service
...
See `dmc -h` for other possible options.
...
ExecStart=/usr/bin/docker run --rm \
  --name %p \
  --link rethinkdb-proxy-28015:rethinkdb \
  -e "dmc_ROOT_GROUP_NAME=YOUR_ROOT_GROUP_NAME_HERE" \
  -e "dmc_ADMIN_NAME=YOUR_ADMIN_NAME_HERE" \
  -e "dmc_ADMIN_EMAIL=YOUR_ADMIN_EMAIL_HERE" \
  opendmc/dmc:latest \
  dmc migrate
```

### `dmc-processor@.service`

**Requires** the `rethinkdb-proxy-28015.service` unit on the same host.

The processor is the main workhorse of dmc. It acts as a bridge between the devices and the app, and nearly all communication goes through it. You may wish to have more than one instance running.
...
ExecStart=/usr/bin/docker run --rm \
  ...
  opendmc/dmc:latest \
  dmc processor %p-%i \
    --connect-app-dealer tcp://appside.dmc.example.org:7160 \
    --connect-dev-dealer tcp://devside.dmc.example.org:7260
ExecStop=-/usr/bin/docker stop -t 10 %p-%i
```

### `dmc-provider@.service`
...
ExecStart=/usr/bin/docker run --rm \
  ...
  dmc provider \
    --name "%H/%i" \
    --connect-sub tcp://devside.dmc.example.org:7250 \
    --connect-push tcp://devside.dmc.example.org:7270 \
    --storage-url https://dmc.example.org/ \
    --public-ip ${COREOS_PRIVATE_IPV4} \
    --min-port=15000 \
    --max-port=25000 \
    --heartbeat-interval 10000 \
    --screen-ws-url-pattern "wss://dmc.example.org/d/%i/<%= serial %>/<%= publicPort %>/"
ExecStop=-/usr/bin/docker stop -t 10 %p-%i
```
...
ExecStart=/usr/bin/docker run --rm \
  ...
  dmc reaper dev \
    --connect-push tcp://devside.dmc.example.org:7270 \
    --connect-sub tcp://appside.dmc.example.org:7150 \
    --heartbeat-timeout 30000
ExecStop=-/usr/bin/docker stop -t 10 %p
```

### `dmc-storage-plugin-apk@.service`
...
This is a template unit, meaning that you'll need to start it with an instance identifier. In this example configuration the identifier is used to specify the exposed port number (i.e. `dmc-storage-plugin-apk@3300.service` runs on port 3300). You can have multiple instances running on the same host by using different ports.

Furthermore, if you're using a self-signed cert, you may have to add `-e "NODE_TLS_REJECT_UNAUTHORIZED=0"` to the `docker run` command. Don't forget to end the line with `\`.

```ini
[Unit]
Description=dmc APK storage plugin
After=docker.service
Requires=docker.service
...
ExecStart=/usr/bin/docker run --rm \
  ...
  -p %i:3000 \
  opendmc/dmc:latest \
  dmc storage-plugin-apk --port 3000 \
    --storage-url https://dmc.example.org/
ExecStop=-/usr/bin/docker stop -t 10 %p-%i
```

### `dmc-storage-plugin-image@.service`

The image storage plugin loads raw blobs from the main storage unit and and allows images to be resized using parameters.

This is a template unit, meaning that you'll need to start it with an instance identifier. In this example configuration the identifier is used to specify the exposed port number (i.e. `dmc-storage-plugin-image@3400.service` runs on port 3400). You can have multiple instances running on the same host by using different ports.

Furthermore, if you're using a self-signed cert, you may have to add `-e "NODE_TLS_REJECT_UNAUTHORIZED=0"` to the `docker run` command. Don't forget to end the line with `\`.

```ini
[Unit]
Description=dmc image storage plugin
After=docker.service
Requires=docker.service
...
ExecStart=/usr/bin/docker run --rm \
  ...
  -p %i:3000 \
  opendmc/dmc:latest \
  dmc storage-temp --port 3000 \
    --save-dir /data
ExecStop=-/usr/bin/docker stop -t 10 %p-%i
```

### `dmc-triproxy-app.service`

This unit provides the `appside.dmc.example.org` service mentioned earlier. Its purpose is to send and receive requests from the app units, and distribute them across the processor units. It's "dumb" in that it contains no real logic, and you rarely if ever need to upgrade the unit.
...
### `dmc-triproxy-dev.service`

This unit provides the `devside.dmc.example.org` service mentioned earlier. Its purpose is to send and receive requests from the provider units, and distribute them across the processor units. It's "dumb" in that it contains no real logic, and you rarely if ever need to upgrade the unit.
...
### `dmc-websocket@.service`

**Requires** the `rethinkdb-proxy-28015.service` unit on the same host.

The websocket unit provides the communication layer between client-side JavaScript and the server-side ZeroMQ+Protobuf combination. Almost every action in dmc goes through the websocket unit.

This is a template unit, meaning that you'll need to start it with an instance identifier. In this example configuration the identifier is used to specify the exposed port number (i.e. `dmc-websocket@3600.service` runs on port 3600). You can have multiple instances running on the same host by using different ports.

Furthermore, if you're using a self-signed cert, you may have to add `-e "NODE_TLS_REJECT_UNAUTHORIZED=0"` to the `docker run` command. Don't forget to end the line with `\`.

```ini
[Unit]
Description=dmc websocket
After=rethinkdb-proxy-28015.service
BindsTo=rethinkdb-proxy-28015.service
...
ExecStart=/usr/bin/docker run --rm \
  ...
  -p %i:3000 \
  opendmc/dmc:latest \
  dmc websocket --port 3000 \
    --storage-url https://dmc.example.org/ \
    --connect-sub tcp://appside.dmc.example.org:7150 \
    --connect-push tcp://appside.dmc.example.org:7170
ExecStop=/usr/bin/docker stop -t 10 %p-%i
```
...
The api unit provides all the major REdmcul APIs for dmc. Users can generate their personal access token from dmc UI and can use that token to access these api from any interface.

This is a template unit, meaning that you'll need to start it with an instance identifier. In this example configuration the identifier is used to specify the exposed port number (i.e. `dmc-api@3700.service` runs on port 3700). You can have multiple instances running on the same host by using different ports.
...
ExecStart=/usr/bin/docker run --rm \
  ...
  --connect-sub tcp://appside.dmc.example.org:7150 \
  --connect-push tcp://appside.dmc.example.org:7170 \
  --connect-sub-dev tcp://devside.dmc.example.org:7250 \
  --connect-push-dev tcp://devside.dmc.example.org:7270
ExecStop=-/usr/bin/docker stop -t 10 %p-%i
```

### `dmc-groups-engine.service`
...
These units are optional and don't affect the way dmc works in any way.

### `dmc-log-rethinkdb.service`
...
```ini
[Unit]
Description=dmc RethinkDB log recorder
After=rethinkdb-proxy-28015.service
BindsTo=rethinkdb-proxy-28015.service
...
### `dmc-notify-hipchat.service`

If you use [HipChat](https://www.hipchat.com/), you can use this unit to push notifications to your room. Check `dmc notify-hipchat --help` for more configuration options.
...
```ini
[Unit]
Description=dmc HipChat notifier
After=docker.service
...
### `dmc-notify-slack.service`

The optional [Slack](https://slack.com/) notifier unit can be enabled to push dmc notifications to a public or private Slack channel. To use it, generate an [API Token](https://api.slack.com/docs/oauth-test-tokens) and select or create a destination channel. Run `dmc notify-slack --help` for additional configuration options.
...
```ini
[Unit]
Description=dmc Slack notifier
After=docker.service
BindsTo=docker.service
...
** NOTE** If you are using this storage, you will not need [dmc-storage-temp@.service](#dmc-storage-tempservice) unit, since both do the same thing. Only the storage location is different.

This is a template unit, meaning that you'll need to start it with an instance identifier. In this example configuration the identifier is used to specify the exposed port number (i.e. `dmc-storage-s3@3500.service` runs on port 3500). Currently, **you cannot have more than one instance of this unit**, as both temporary files and an in-memory mapping is used. Using a template unit makes it easy to set the port.

```ini
[Unit]
Description=dmc s3 storage
After=docker.service
Requires=docker.service
...
If you want to play with dmc API against your dmc platform using swagger UI tool through a web access, then you can use this optional unit. In this example, the unit requires to put the dmc swagger file `api_v1.yaml` to the `/opt/dmc/swagger` folder of the host. You can have multiple instances running on the same host by using different ports.
...
| Unit | IP | Port |
|------|----|------|
| [dmc-app@3100.service](#dmc-appservice) | 192.168.255.100 | 3100 |
| [dmc-auth@3200.service](#dmc-authservice) | 192.168.255.150 | 3200 |
| [dmc-storage-plugin-apk@3300.service](#dmc-storage-plugin-apkservice) | 192.168.255.100 | 3300 |
| [dmc-storage-plugin-image@3400.service](#dmc-storage-plugin-imageservice) | 192.168.255.100 | 3400 |
| [dmc-storage-temp@3500.service](#dmc-storage-tempservice) | 192.168.255.100 | 3500 |
| [dmc-websocket@3600.service](#dmc-websocketservice) | 192.168.255.100 | 3600 |
| [dmc-api@3700.service](#dmc-apiservice) | 192.168.255.100 | 3700 |
...
| Unit | IP | Identifier |
|------|----|------------|
| [dmc-provider@floor4.service](#dmc-providerservice) | 192.168.255.200 | floor4 |
| [dmc-provider@floor8.service](#dmc-providerservice) | 192.168.255.201 | floor8 |

Our base nginx configuration for `dmc.example.org` would then be:
...
http {
  ...
  server {
    ...
    # Handle dmc-provider@floor4.service
    ...
    # Handle dmc-provider@floor8.service
    ...
    location /auth/ {
      proxy_pass http://dmc_auth/auth/;
    }

    location /api/ {
      proxy_pass http://dmc_api/api/;
    }

    location /s/image/ {
      proxy_pass http://dmc_storage_image;
    }

    location /s/apk/ {
      proxy_pass http://dmc_storage_apk;
    }

    location /s/ {
      client_max_body_size 1024m;
      client_body_buffer_size 128k;
      proxy_pass http://dmc_storage;
    }

    location /socket.io/ {
      proxy_pass http://dmc_websocket;
      proxy_http_version 1.1;
      proxy_set_header Upgrade $http_upgrade;
      proxy_set_header Connection $connection_upgrade;
...
```ini
[Unit]
Description=dmc nginx public load balancer
After=docker.service
Requires=docker.service
ConditionPathExists=/srv/ssl/dmc.example.org.crt
ConditionPathExists=/srv/ssl/dmc.example.org.key
...
ExecStart=/usr/bin/docker run --rm \
  --name %p \
  --net host \
  -v /srv/ssl/dmc.example.org.crt:/etc/nginx/ssl/cert.pem:ro \
  -v /srv/ssl/dmc.example.org.key:/etc/nginx/ssl/cert.key:ro \
