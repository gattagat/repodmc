...
# Sneak the dmc executable into $PATH.
...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    rm node-v*.tar.xz && \
    su dmc-build -s /bin/bash -c '/usr/local/lib/node_modules/npm/node_modules/node-gyp/bin/node-gyp.js install' && \
...
RUN mkdir -p /app && \
    chown -R dmc-build:dmc-build /tmp/build /app
...
RUN set -x && \
    ...
    npm pack && \
    tar xzf dmc-*.tgz --strip-components 1 -C /app && \
    bower cache clean && \
