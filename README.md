<img src="res/common/logo/exports/dmc-128.png?raw=true" style="width:100px;height:100px;" alt="dmc">

[![Build Status](https://travis-ci.org/opendmc/dmc.svg?branch=master)](https://travis-ci.org/opendmc/dmc)
[![Docker Pulls](https://img.shields.io/docker/pulls/opendmc/dmc.svg)](https://hub.docker.com/r/opendmc/dmc/)
[![NPM version](https://img.shields.io/npm/v/dmc.svg)](https://www.npmjs.com/package/dmc)
[![Backers on Open Collective](https://opencollective.com/opendmc/backers/badge.svg)](#backers) [![Sponsors on Open Collective](https://opencollective.com/opendmc/sponsors/badge.svg)](#sponsors)

**dmc** (or Smartphone Test Farm) is a web application for debugging smartphones, smartwatches and other gadgets remotely, from the comfort of your browser.

dmc was originally developed at [CyberAgent](https://www.cyberagent.co.jp/en/) to control a growing collection of more than 160 devices. As of July 2016 development is mainly sponsored by [HeadSpin](https://performance.headspin.io/) and [other individual contributors](https://opencollective.com/opendmc).

We  welcome financial contributions in full transparency on our [open collective](https://opencollective.com/opendmc).
...
Thank you to all our sponsors! (please ask your company to also support this open source project by [becoming a sponsor](https://opencollective.com/opendmc#sponsor))
...
> [HeadSpin](https://headspin.io/) provides secure and scalable dmc for iOS integrated with Appium/XCTest/Selenium/Espresso, High speed interaction Audio/Video/Game testing and AI based Root cause analysis for Performance Management. It's free to start using HeadSpin in 150+ locations worldwide! [Try it out for free.](https://ui.headspin.io/register?referral=start-testing-hs)

HeadSpin offers a generous monthly contribution towards dmc development.

### Contributors
Thank you to all the people who have already contributed to dmc!
<a href="../../graphs/contributors"><img src="https://opencollective.com/opendmc/contributors.svg?width=890" /></a>

### Backers
Thank you to all our backers! [[Become a backer](https://opencollective.com/opendmc#backer)]

<a href="https://opencollective.com/opendmc#backers" target="_blank"><img src="https://opencollective.com/opendmc/backers.svg?width=890"></a>


### How to become a sponsor

Please use our [open collective](https://opencollective.com/opendmc#sponsor) or [contact us][contact-link] directly for sponsor arrangements. Both recurring and one-time contributions are most welcome. Contributions towards a specific issue or feature are also possible, and can be attributed to your company in our release notes and other related materials. Hardware-only contributions, whether new or used, are also extremely helpful and well received, especially if you find a device that doesn't work. Please see our [donation transparency report](DONATION-TRANSPARENCY.md) for past hardware contributions.
...
* Remote control any device from your browser
  - Real-time screen view
    * Refresh speed can reach 30-40 FPS depending on specs and Android version. See [minicap](https://github.com/opendmc/minicap) for more information.
    * Rotation support
  ...
  - Multitouch support on touch screens via [minitouch](https://github.com/opendmc/minitouch), two finger pinch/rotate/zoom gesture support on regular screens by pressing `Alt` while dragging
  - Drag & drop installation and launching of `.apk` files
    * Launches main launcher activity if specified in the manifest
  - Reverse port forwarding via [minirev](https://github.com/opendmc/minirev)
...
We highly encourage open participation in the community. However, if you're running a business that uses dmc or would like to use dmc, you may sometimes want to have an expert, i.e. one of the original developers or a skilled contributor, work with you to set up a prototype for evaluation purposes, add support for new or old hardware, figure out an issue, fix a bug or add some new feature. Our services are similar to [FFmpeg's](https://ffmpeg.org/consulting.html). [Contact us][contact-link] with details and we'll see what we can do.
...
Note that you need these dependencies even if you've installed dmc directly from [NPM](https://www.npmjs.com/), because they can't be included in the package.
...
On Windows you're on your own. In theory you might be able to get dmc installed via [Cygwin](https://www.cygwin.com/) or similar, but we've never tried. In principle we will not provide any Windows installation support, but please do send a documentation pull request if you figure out what to do.

We also provide a [Docker](http://docker.com/) container in the [Docker Hub](https://hub.docker.com/) as [opendmc/dmc](https://registry.hub.docker.com/u/opendmc/dmc/). You can use our [Dockerfile](Dockerfile) as guidance if you'd prefer to do the installation yourself.

You should now be ready to [build](#building) or [run](#running) dmc.
...
You may also wish to link the module so that you'll be able to access the `dmc` command directly from the command line:
...
dmc comprises of several independent processes that must normally be launched separately. In our own setup each one these processes is its own [systemd](http://www.freedesktop.org/wiki/Software/systemd/) unit. See [DEPLOYMENT.md](doc/DEPLOYMENT.md) and [Setup Examples](https://github.com/opendmc/setup-examples) if you're interested.
...
An administrator level is available in dmc in addition of the native user one, with increased rights on some features (e.g.  booking & partitioning systems, management of users & devices, ...). The corresponding built-in administrator user has the following default credentials:
- name: `administrator`
- email: `administrator@fakedomain.com`

Another built-in object exists, this is the root standard group to which the users and devices belong the first time they register to the dmc database, its default name is `Common`

These built-in objects are created in the dmc database if they do not already exist 

Of course, you can override the default values of these built-in objects by settings the following environment variables before to initialize the dmc database through `dmc local` or `dmc migrate` commands:
-	root standard group name: `dmc_ROOT_GROUP_NAME` 
-	administrator user name: `dmc_ADMIN_NAME`
-	administrator user email: `dmc_ADMIN_EMAIL`

You're now ready to start up dmc itself:

```bash
dmc local
```

After the [webpack](http://webpack.github.io/) build process has finished (which can take a small while) you should have your private dmc running on [http://localhost:7100](http://localhost:7100). If you had devices connected before running the command, those devices should now be available for use. If not, you should see what went wrong from your console. Feel free to plug in or unplug any devices at any time.

Note that if you see your device ready to use but without a name or a proper image, we're probably missing the data for that model in [our device database](https://github.com/opendmc/dmc-device-db). Everything should work fine either way.

If you want to access dmc from other machines, you can add the `--public-ip` option for quick testing.

```bash
dmc local --public-ip <your_internal_network_ip_here>
```
...
### Can I deploy dmc to actual servers?

Yes, see [DEPLOYMENT.md](doc/DEPLOYMENT.md) and [Setup Examples](https://github.com/opendmc/setup-examples).

### Will I have to change battery packs all the time?

No, not all the time. Aside from a single early failure we had within only a few months, all of our devices were doing fine for about two years. However, having reached the 2-3 year mark, several devices have started to experience visibly expanded batteries. Expanded batteries should be replaced as soon as possible. Note that this issue isn't specific to dmc, it's just what happens over time. You should be prepared to replace the batteries every now and then. In any case, we consider 2 years per battery pack to be fairly good value for a device lab.

You should set up your devices so that the display is allowed to turn off entirely after a short timeout. 30 seconds or so should do just fine, dmc will wake it up when necessary. Otherwise you risk reducing the lifetime of your device.
...
### How do I uninstall dmc from my device?

When you unplug your device, all dmc utilities except dmcService stop running automatically. It doesn't do any harm to force stop or uninstall it.

To uninstall the dmcService, run the following command:

```bash
adb uninstall jp.co.cyberagent.dmc
```

You may also wish to remove our support binaries, although as mentioned before they won't run unless the device is actually connected to dmc. You can do this as follows:
...
* The device might not have a unique USB serial number, causing dmc to overwrite the other device instead
...
Currently dmc UI is available in English and Japanese.

If you would like translate to any other language, please contribute in the [dmc Transifex project](https://www.transifex.com/projects/p/dmc/).
...
2. Extract with gettext all translatable strings to `dmc.pot`.
3. Push `dmc.pot` to Transifex.
...
Copyright © 2017 The Opendmc Project. All Rights Reserved.

[contact-link]: mailto:contact@opendmc.io
